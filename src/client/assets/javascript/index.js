...
async function handleCreateRace() {
	try {
		...
		if (track_id === undefined || player_id === undefined) {
			alert('Track or racer is not selected')
			return;
		}
		...
		console.group(`Error at handleCreateRace`);
		console.log(error);
		console.groupEnd();
	}
}
	...
	return new Promise(resolve => {
		...
		const raceInterval = setInterval(async () => {
			try {
				...
				const {status, positions} = raceInfo;
				/* 
				...
				*/
				...
				console.log(`Problem with raceInterval request::`, error)
			}
		}, 500)
	})
	.catch(error => `Problem with runRace(${raceID}) request::`, error)
...
function handleSelectPodRacer(target) {
	...
	Object.assign(store, { player_id: parseInt(target.id) })
}
...
function handleSelectTrack(target) {
	...
	Object.assign(store, { track_id: parseInt(target.id) })
}
...
function getRace(id) {
	...
	return fetch(`${SERVER}/api/races/${id}`)
		.then(response => response.json())
		.then(data => data)
		.catch(error => console.log(`Problem with getRace(${id}) request::`, error))
}
